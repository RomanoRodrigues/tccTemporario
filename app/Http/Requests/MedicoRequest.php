...
class MedicoRequest extends FormRequest
{
    ...
    {
        $medicoId = $this->route('medicos');

        return [
             'nome' => ['required'],
            'telefone' => ['required', 'min:11'],
            'email' => ['required','email', Rule::unique('pacientes')->ignore($medicoId)->whereNull('deleted_at')],
            'endereco' => ['required'],
            'cpf' => ['required', Rule::unique('pacientes')->ignore($medicoId)->whereNull('deleted_at')],
